...
module Jekyll
  ...
  class PeoplePagesGenerator < Generator
    safe true

    # generate loops over _config.yml/people_page invoking the PeoplePage
    # constructor for each record for which we want to generate a page

    def generate(site)
      # people_page_dirs determines whether we want to generate index pages
      # (filename/index.html) or standard files (filename.html). This information
      # is passed to the PeoplePage constructor, which sets the @dir variable
      # as required by this directive
      index_files = site.config['people_page-dirs'] == true
        ...
        data.each do |data_spec|
          ...
          if site.layouts.key? template
            ...
            records.each do |record|
              site.pages << PeoplePage.new(site, site.source, index_files, dir, record, filename, template, extension)
            end
          else
  ...
  module PeoplePageLinkGenerator
    include Sanitizer

    # use it like this: {{input | peoplepage_url: dir}}
    ...
    def peoplepage_url(input, dir)
      @gen_dir = Jekyll.configuration({})['people_page-dirs']
      if @gen_dir then
...
Liquid::Template.register_filter(Jekyll::PeoplePageLinkGenerator)
